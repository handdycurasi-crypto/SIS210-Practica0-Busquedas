...
\section{Estado del arte y fundamento teórico}
Buscar una clave en un arreglo exige distinguir las garantías del algoritmo de las características de la entrada. Lineal admite datos sin ordenar y revisa elementos sucesivos; binaria exige orden y descarta aproximadamente la mitad del intervalo por iteración. Los textos de Cormen et al. (2022) y Sedgewick y Wayne (2011) sitúan esta comparación dentro del análisis de crecimiento y del estudio experimental: contar operaciones y medir duración responden preguntas complementarias.

Exponencial obtiene una cota mediante índices $1,2,4,8,\ldots$ y aplica binaria al intervalo delimitado. Su costo depende de la posición alcanzada, $O(\log(p+1))$, y resulta útil cuando existe acceso indexado seguro pero no se conoce inicialmente el final. Interpolación estima
\[
\text{pos}=\ell+\left\lfloor\frac{(x-a[\ell])(h-\ell)}{a[h]-a[\ell]}\right\rfloor,
\]
con controles de rango y de extremos iguales. Perl et al. (1978) vinculan su costo esperado de accesos, $O(\log\log n)$, a claves uniformemente distribuidas; una distribución sesgada puede invalidar esa ventaja. Van Sandt et al. (2019) estudian versiones optimizadas que intercambian cálculo por menos accesos a memoria y adaptan la búsqueda a la distribución. Esto amplía el enfoque clásico: ni el nombre del algoritmo ni su orden asintótico bastan para anticipar su rendimiento práctico. Aquí se evalúan versiones manuales básicas, no esas variantes optimizadas.

\textbf{Notación.} $O(g(n))$ acota superiormente una función de costo hasta una constante, para $n$ suficientemente grande; $\Omega(g(n))$ la acota inferiormente y $\Theta(g(n))$ reúne ambas cotas. No equivalen, respectivamente, a peor, mejor y promedio: los casos se definen por separado. Bajo un modelo RAM de costo unitario, las versiones iterativas usan $O(1)$ espacio auxiliar.
...
Exponencial & $\Theta(1)$ & $\Theta(\log n)$ & $\Theta(\log n)$ & Mismo modelo; claves próximas al inicio cuestan menos.\\
...
\section*{Referencias}
\begingroup\fontsize{8}{9}\selectfont\setlength{\parskip}{3pt}
\newcommand{\referencia}[1]{\par\hangindent=1.1em\hangafter=1 #1\par}
...
\referencia{Van Sandt, P., Chronis, Y., \& Patel, J. M. (2019). Efficiently searching in-memory sorted arrays: Revenge of the interpolation search? In \textit{Proceedings of the 2019 International Conference on Management of Data} (pp. 36--53). Association for Computing Machinery. \url{https://doi.org/10.1145/3299869.3300075}}
